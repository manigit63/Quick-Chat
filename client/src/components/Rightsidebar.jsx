...
const Rightsidebar = () => {
  ...
  //get all the images from the messages and set them to the states

  useEffect(() => {
    if (Array.isArray(message)) {
      const images = message.filter((msg) => msg.image).map((msg) => msg.image);
      setMsgImage(images);
    } else {
      setMsgImage([]); // fallback if message is not an array
    }
  }, [message]);
